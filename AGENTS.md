...
- **`src/editor.ts`** — `renderEditor(pathname, actionQuery)`: CodeMirror-based in-browser file editor page, reached via `?action=edit`. It is part of the management/listing surface: `?action=edit` is only served when the `list` feature is enabled and is guarded by `list`'s access level (auth for `"private"`, open for `"public"`). When `list` is disabled the action query is ignored and the request is served as a normal file (governed by `browser`). The activating query key is configurable via `WedbavOptions.actionQuery` (default `"action"`) to avoid clashing with a user app's own `?action`. `?action=edit` on a **directory** forces the listing/manager UI even when the directory has an `index.html` (a non-slash dir path 308-redirects to the trailing-slash form first).
- **`src/zip.ts`** — Directory-tree zip/unzip. `zipDirectory(fs, dir)` and `unzipInto(fs, dir, bytes)` use the optional `FsSubset._readDirMany`/`_writeDirMany` batch helpers (single query/transaction) with a `readdir`+`readFile` / `mkdir`+`writeFile` fallback. Also hosts the HTTP handlers (`handleZipDownload`/`handleZipUpload`). The feature is driven by the `actionQuery` (default `?action=`, resolved in `wedbav.ts`): `GET /dir/?action=download` streams a zip of the tree (guarded by `list` auth; `?action=download` on a **file** forces a plain attachment download via `serveAttachment`); `PUT /dir/?action=extract` unzips the body into the dir (behind global basic auth). Because it's URL-navigable, the manager's directory download button is a plain `<a href="…?action=download">`. Zip-slip (`..`/absolute paths) is rejected with 400. Uses `fflate` (pure-JS, multi-runtime).
...
- **Browser & list modes**: Two independent options, each `"public" | "private" | "false" | false` (`resolveBrowserFeatures` in `wedbav.ts` normalizes them). `browser` (env `WEDBAV_BROWSER`, default `"private"`) controls the file-serving feature; `list` (env `WEDBAV_LIST`, defaults to whatever `browser` resolves to) controls the directory auto-listing UI. `"private"` requires basic auth, `"public"` does not, `false`/`"false"` disables the feature. File serving and listing carry **independent** auth requirements (e.g. `browser:"public"` + `list:"private"` = public files but an auth-gated listing). `list` only takes effect when `browser` is enabled; when `browser` is `false` the listing is forced off and GET falls through to WebDAV semantics. With listing disabled, directories without an `index.html` return 404.
