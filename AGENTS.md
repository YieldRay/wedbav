...
- **`src/editor.ts`** — `renderEditor(pathname, editQuery)`: CodeMirror-based in-browser file editor page (`?edit`). It is part of the management/listing surface: `?edit` is only served when the `list` feature is enabled and is guarded by `list`'s access level (auth for `"private"`, open for `"public"`). When `list` is disabled the `?edit` query is ignored and the request is served as a normal file (governed by `browser`). The activating query key is configurable via `WedbavOptions.editQuery` (default `"edit"`) to avoid clashing with a user app's own `?edit`. `?edit` on a **directory** forces the listing/manager UI even when the directory has an `index.html` (a non-slash dir path 308-redirects to the trailing-slash form first).
