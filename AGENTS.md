...
- **Browser mode**: Controlled by `WEDBAV_BROWSER` env (`private` | `public` | `enabled` | `disabled`); default is `private`. `private` requires auth and renders the directory-listing HTML UI; `public` is the same but without auth; `enabled` is like `public` (no auth) but does not render the listing UI (directories without an `index.html` return 404); `disabled` is like `private` (requires auth) but does not render the listing UI.
